				...
				<div class="d-flex align-items-center mb-2">
					<div class="d-flex w-25">
						...
						<c:if test="${sort eq 'popular'}">
							<a class="me-3 btn btn-sm btn-outline-primary"
								href="/board?category=${category}&sort=latest">최신</a>
							<a class=" btn btn-sm btn-outline-danger active"
								href="/board?category=${category}&sort=popular">인기</a>
						</c:if>
						<!-- 최신버튼 클릭시 -->
						<c:if test="${sort ne 'popular'}">
							<a class="me-3 btn btn-sm btn-outline-primary active"
								href="/board?category=${category}&sort=latest">최신</a>
							<a class="btn btn-sm btn-outline-danger"
								href="/board?category=${category}&sort=popular">인기</a>
						</c:if>
					</div>
					<form class="input-group ms-auto" style="width: 40%"
						action="/board">
						<select class="form-select form-select-sm" id="condition"
							...
							id="keyword" name="keyword" placeholder="검색" maxlength="100"
							style="width: 30%">
								...
								<c:forEach var="board" items="${boardList}">
									<tr>
										...
										<td style="width: 27%; max-width: 100px"><a
											href="/board/detail/${board.boardSeq}"
										...
										<td class="text-center" style="width: 10%"><c:choose>
												...
												<c:otherwise>
													<fmt:formatDate value="${wrtDate}" pattern="yy-MM-dd" />
												</c:otherwise>
											</c:choose></td>
							...
							<ul class="pagination">
								...
								<li class="page-item ${currentNumber == 0 ? 'disabled' : ''}">
									<c:if test="${!empty keyword}">
										<a class="page-link"
											href="/board?category=${category}&sort=${sort}&page=${currentNumber - 1}&condition=${condition}&keyword=${keyword}">이전</a>
								...
								<c:forEach var="pageNumber" begin="0" end="${totalPages - 1}">
									<li
										...
										<!-- 페이지 번호 출력 시에 1을 더해줌 --> <c:if test="${!empty keyword}">
											<a class="page-link"
												href="/board?category=${category}&sort=${sort}&page=${pageNumber}&condition=${condition}&keyword=${keyword}">${pageNumber + 1}</a>
								...
								<li
									...
									<c:if test="${!empty keyword}">
										<a class="page-link"
											href="/board?category=${category}&sort=${sort}&page=${currentNumber + 1}&condition=${condition}&keyword=${keyword}">다음</a>
									</c:if> <c:if test="${empty keyword}">
										<a class="page-link"
